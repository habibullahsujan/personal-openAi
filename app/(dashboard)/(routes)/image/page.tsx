  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (error?.response?.status === 403) {
        ...
        toast.error('Image generation error form open ai.')
      }
    } finally {
      router.refresh();
    }
  ...
  return (
    <div>
      <Heading
        ...
        description="Turn prompt to a image."
        icon={ImageIcon}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-8">
            {images.map((src) => (
              <Card key={src}>
                ...
                <CardFooter>
                  <Button onClick={()=>window.open(src)} variant={"secondary"} className="w-full flex items-center justify-center gap-2">
